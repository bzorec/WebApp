...
function username_exists($username)
{
    ...
    $query = "SELECT * FROM vaja1.users WHERE username='$username'";
    $res = $conn->query($query);
    return mysqli_num_rows($res) > 0;
}

// Funkcija ustvari uporabnika v tabeli users. Poskrbi tudi za ustrezno šifriranje uporabniškega gesla.
function register_user($username, $password, $naslov, $phone, $email)
{
    global $conn;
    $username = mysqli_real_escape_string($conn, $username);
    $pass = sha1($password);
    ...
    */
    $query = "INSERT INTO vaja1.users (username, password, email, phone, naslov) VALUES ('$username', '$pass', '$email', '$phone', '$naslov');";
    if ($conn->query($query)) {
        return true;
    } else {
